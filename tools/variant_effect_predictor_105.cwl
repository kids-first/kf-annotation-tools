...
arguments:
  ...
  - position: 10
    shellQuote: false
    valueFrom: >-
      && tabix $(inputs.output_basename).$(inputs.tool_name).vep_annotated.vcf.gz'
...
inputs:
  ...
  compress_output: { type: 'string?', inputBinding: { position: 2, prefix: "--compress_output" }, doc: "Writes output compressed using either gzip or bgzip.", default: "bgzip" }
  ...
  fork: {type: 'int?', default: 16, doc: "Number of forks (cores) to use. May need to increase for really large inputs",
  ...
  output_file: { type: 'string?', doc: "Output name of main result file. Use STDOUT to pipe to something else", default: results.vep.vcf.gz,
